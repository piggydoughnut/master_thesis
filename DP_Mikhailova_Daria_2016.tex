...
REST API design is an important part of the analysis process. It helps defining application's functions and capabilities and develops an overall understanding of the workflow.
...
\item {OAuth} 
	...
	All of the above listed endpoints are only accessed with a valid token.
\end{itemize}
...
Example of the request with authorisation grant which is described in the step 4 is shown on listing \ref{lst:json}. \\~\\
\verb|POST| \url{https://oauth.example.com/api/oauth/token}
\begin{listing}
...
\end{lstlisting}
\caption{Request body of user authorisation example} 
\label{lst:json}
\end{listing}
